...
export function FileUploader({ onFilesSelected }: FileUploaderProps) {
    ...
    const handleLocalGenerateDepthMap = useCallback(async (file?: File) => {
        const targetFile = file || imageFile;
        if (!targetFile || !workerRef.current) return;
    ...
    const handleRemoteGenerateDepthMap = useCallback(async (currentApiUrl: string, file?: File) => {
        const targetFile = file || imageFile;
    ...
    }, [imageFile, apiUrl, defaultApiUrl, toast]);

    // 自动生成深度图函数
    const autoGenerateDepthMap = useCallback((imageFile: File) => {
        if (useLocalGenerator) {
            handleLocalGenerateDepthMap(imageFile);
        } else {
            handleRemoteGenerateDepthMap(apiUrl || defaultApiUrl, imageFile);
        }
    }, [useLocalGenerator, apiUrl, defaultApiUrl, handleLocalGenerateDepthMap, handleRemoteGenerateDepthMap]);

    // 自动触发深度图生成
    useEffect(() => {
        if (imageFile && !depthMapFile && !isGenerating && !isLocalGenerating) {
            autoGenerateDepthMap(imageFile);
        }
    }, [imageFile, depthMapFile, isGenerating, isLocalGenerating, autoGenerateDepthMap]);
    ...
    }, [imageFile, depthMapFile, onFilesSelected]);
